  ...
  switch (name) {
    case "list_components": {
      const list = Object.entries(components)
        .map(([key, val]: [string, any]) => `• **${key}**: ${val.description}`)
        .join('\n');
    ...
    case "get_component": {
      ...
      if (comp.examples && comp.examples.length > 0) {
        output += `## Exemplos\n\n`;
        comp.examples.forEach((example: string, index: number) => {
    ...
    case "get_template": {
      const { template, entityName = "items" } = args as any;
